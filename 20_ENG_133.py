...
def open_subtopic_window(category):
    ...
    header.pack(fill=X)
    Label(header, text=f"🧠 CMT Tree Viewer - {category}", font=("Arial", 16, "bold"), bg="#004080", fg="white").pack(pady=10)

    content = Frame(new_win, bg="white")
    content.pack(fill=BOTH, expand=True, padx=20, pady=20)

    if category in ["Diseases", "Drugs", "Anatomy"]:
        Label(content, text=f"Select a {category[:-1]}:", font=("Arial", 12), bg="white").pack(anchor=W)

        json_files = {
            "Diseases": "C:\\Users\\DELL\\.vscode\\medical informatics\\disease_icd [MConverter.eu].json", #ICD_Source
            "Drugs": "C:\\Users\\DELL\\.vscode\\medical informatics\\drug_tree.json",#RXNorm_Source
            "Anatomy": "C:\\Users\\DELL\\.vscode\\medical informatics\\anatomy_tree_snomed.json" #SNOMED_Source
        }

        try:
            with open(json_files[category], "r", encoding="utf-8") as f:
                data = json.load(f)[category]
        except Exception as e:
            Label(content, text=f"Error loading {category.lower()} data: {e}", fg="red", bg="white").pack()
            return

        selected_item = StringVar()
        item_dropdown = ttk.Combobox(content, textvariable=selected_item, state="readonly", font=("Arial", 11), width=40)
        ...
        def build_tree(tree_widget, parent, items):
            for item in items:
                node = tree_widget.insert(parent, 'end', text=f"{item['code']} - {item['title']}")
                if "children" in item:
        ...
        subtopics = {
            "Diagnostic Techniques": ["MRI", "Ultrasound", "ECG", "CT Scan"]
        }

        Label(content, text="Select a subtopic:", font=("Arial", 11), bg="white").pack(anchor=W)
        subtopic_var = StringVar()
        subtopic_dropdown = ttk.Combobox(content, textvariable=subtopic_var, state="readonly", font=("Arial", 11), width=30)
        subtopic_dropdown['values'] = subtopics.get(category, [])
        subtopic_dropdown.pack(pady=10, anchor=W)
        Label(content, text="(Tree view will appear here)", font=("Arial", 10, "italic"), bg="white", fg="gray").pack(pady=20, anchor=W)
...
Label(root, text="Welcome to the Controlled Medical Terminology Viewer!",
      font=("Arial", 16, "bold"), bg="#E6F2FA", fg="black").pack(pady=(20, 10))
Label(root, text="Explore unique clinical categories with structured medical knowledge.",
      font=("Arial", 11), bg="#E6F2FA", fg="#333333").pack()
